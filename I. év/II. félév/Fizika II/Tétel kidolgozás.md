...
3. Egy S felületű egyenáram által átjárt vezető keret esetében adja meg a keret mágneses nyomatékának kifejezését (vektoriálisan és skalárisan). Amennyiben ez a vezető keret egy homogén mágneses térben kerül, amelynek az iránya merőleges a vezető keret forgástengelyére, adja meg a vezető keretre ható forgató nyomatékot (vektoriálisan és skalárisan). Adja meg a maximális forgató nyomaték megjelenésének feltételét, majd ebből kiindulva adja meg a mágneses tér indukció mértékegységének definícióját.
...
- **A vezető keretre ható forgatónyomaték ($\vec{M}$):**
...
6. A görbületi-sugár és a töltéssűrűség közötti összefüggés levezetése és értelmezése. Szemléltessen ábrán!
...
8. Adja meg a mágneses Gauss-törvény és Faraday-törvény differenciális alakját és adja meg mindegyik egyenlet fizikai jelentését (készítsen ábrákat a fizikai értelmezések szemléltetéséhez!).
...
2. Egy $\vec{p}$ dipólus nyomatékkal rendelkező elektromos dipólus, $\vec{E}$ térerősségű elektrosztatikus térben helyezkedik el úgy, hogy a két vektor közötti szög $\alpha$. A forgató nyomaték definíciós képletéből kiindulva vezesse le a forgató nyomaték $\vec{p}$ és $\vec{E}$ vektorokkal megadott kifejezést.
...
![Gauss felület 1](Images/gauss-felulet1.png)
![Gauss felület 2](Images/gauss-felulet2.png)
4. Tekintsen egy végtelenül hosszú és elhanyagolható vastagságú vezetéket, amelyben I erősségű áram folyik. Ha a vezeték egy B indukciójú homogén mágneses térben van úgy, hogy a mágneses indukció merőleges a vezetékre, akkor kiindulva az elektronokra ható Lorentz-erő kifejezéséből, vezesse le az erő kifejezését a mágneses tér indukció, a vezeték egységnyi hosszúsága és a vezetékben folyó áram függvényében.
   
   A mágneses térben mozgó egyetlen elektronra ható Lorentz-erő:$$\vec{F}_e = -e(\vec{v}_d \times \vec{B})$$(ahol $\vec{v}_d$ a driftsebesség, $e$ az elemi töltés).

Tekintsünk a vezetékből egy $l$ hosszúságú, $A$ keresztmetszetű darabot. A benne lévő szabad elektronok száma: $N = n A l$, ahol $n$ az elektronsűrűség.

Az összes elektronra ható eredő erő (ez fog a rácson keresztül a vezetékre hatni):$$\vec{F} = N \cdot \vec{F}_e = (n A l) \left[ -e (\vec{v}_d \times \vec{B}) \right]$$Az áramerősség ($I$) az egységnyi idő alatt átáramló töltés. A vezeték $I\vec{l}$ árameleme kifejezhető a töltéshordozók sebességével:$$I\vec{l} = -n A e l \vec{v}_d$$
_(A mínusz jel azért kell, mert az áram iránya ellentétes az elektronok haladási irányával)._ Ezt behelyettesítve:
$$\vec{F} = I\vec{l} \times \vec{B}$$
Mivel a mágneses indukció merőleges a vezetékre ($\vec{l} \perp \vec{B}$), a vektoriális szorzat nagysága a szinusz ($90\text{ fok} = 1$) miatt: $F = I l B$.
Az egységnyi hosszúságra ható erő (eloszlott erő):$$\vec{f} = \frac{\vec{F}}{l} = I(\vec{u}_l \times \vec{B}) \implies |\vec{f}| = I B$$

5. Mutasssa be milyen erőhatást gyakorol egymásra két végtelen hosszúságú végtelenül vékonynak tekintett egyenes vezető, amelyekben azonos irányú áramok folynak! Készítsen ábrát és tüntessen fel minden szükséges mennyiséget!
...
8. Adja meg az elektromos Gauss-törvény és az Ampére-törvény (kiegészítve az eltolási áramsűrűséggel) differenciális alakját és adja meg mindegyik egyenlet fizikai jelentését (készítsen ábrákat a fizikai értelmezések szemléltetéséhez!).
   
   - **Elektromos Gauss-törvény:**
    
    $$\nabla \cdot \vec{E} = \frac{\rho}{\varepsilon_0}$$
    
    _Fizikai jelentés:_ Az elektromos erőtér forrásai az elektromos töltések. A pozitív töltések a térerősség forrásai (divergencia pozitív), a negatív töltések a nyelői.
    
- **Kibővített Ampère-törvény (Ampère-Maxwell törvény):**    $$\nabla \times \vec{B} = \mu_0 \vec{J} + \mu_0 \varepsilon_0 \frac{\partial \vec{E}}{\partial t}$$
    _Fizikai jelentés:_ Mágneses örvényteret (rotációt) nemcsak az áramló töltések (vezetési áramsűrűség, $\vec{J}$) képesek létrehozni, hanem az időben változó elektromos tér is, amelyet Maxwell eltolási áramsűrűségnek ($\varepsilon_0 \frac{\partial \vec{E}}{\partial t}$) nevezett.